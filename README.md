...
LocalScribe is a native, local-first macOS transcription app for microphone input, media files, live captions, transcript editing, subtitle import/export, and optional on-device translation.

- Apple Speech is the default engine on macOS 26 and later.
- whisper.cpp runs through Metal with a CPU fallback and bundled Silero VAD.
- SenseVoice and NVIDIA Parakeet are available for offline file transcription.
- Apple Translation is the default post-transcription translator; NLLB is optional.
- Audio and imported transcripts remain on the Mac. Network access is used only for user-initiated model downloads, update checks, and external documentation.

The current source version is **1.3.6 (18)** and supports Apple silicon on macOS 15.5 or later. Apple SpeechAnalyzer and live captions require macOS 26. A Developer ID-signed and notarized public binary has not been released yet; developers can build the app from source using the instructions above.
